...
try {
    ...
    if ($post['IsSoftDeleted'] == 1) {
        throw new Exception('Пост №' . $post['IdPost'] . ' уже удален!');
    }

    $sql = 'UPDATE Posts SET IsPublished = ?, IsSoftDeleted = ?, SoftDeletedAt = ? WHERE IdPost = ?';
    $conn->prepare($sql)->execute([0, 1, date('Y-m-d H:i:s'), $post['IdPost']]);
    ...
    LogAction(basename(__FILE__), __DIR__ . '/../logs/services.log', "SUCCESS", "Пост №{$post['IdPost']} успешно удален!");
    echo json_encode(['success' => true, 'message' => 'Пост №' . $post['IdPost'] . ' успешно удален!']);
} catch (Exception $e) {
